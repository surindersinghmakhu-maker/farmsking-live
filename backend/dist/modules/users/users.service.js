...
let UsersService = class UsersService {
    ...
    async becomeFarmer(user, dto) {
        ...
        const rolesToAdd = isPartnerDeactivated ? [client_1.Role.FARMER] : [client_1.Role.FARMER, client_1.Role.BUSINESS_PARTNER];
        const newRoles = Array.from(new Set([...currentRoles, ...rolesToAdd]));
        const newDeactivated = currentDeactivated.filter((r) => r !== client_1.Role.FARMER);
        const profileData = {};
        if (dto) {
