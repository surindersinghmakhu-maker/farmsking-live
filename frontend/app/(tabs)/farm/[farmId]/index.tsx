...
export default function FarmDetailScreen() {
  ...
  const onAddCrop = async () => {
    setError(null);
    const area = Number(plotSize);
    const plants = plantCount ? Number(plantCount) : undefined;

    if (!plotName.trim() || !area || area <= 0) {
      setError('Enter a plot name and a valid plot size.');
      return;
    }
    if (!cropName.trim()) {
    ...
    try {
      const plot = await createPlot.mutateAsync({ farmId: farmId as string, name: plotName.trim(), area, areaUnit: 'ACRE' });
      ...
      setError(err?.response?.data?.message ?? 'Could not save this crop. Please try again.');
    }
  };
  ...
  return (
    <View style={styles.container}>
      <LinearGradient colors={theme.gradient} style={styles.hero}>
        ...
        <Text style={styles.heroTitle}>{farm?.name ?? 'Farm'}</Text>
        {farm ? (
          <Text style={styles.heroSubtitle}>
            {farm.totalArea} {farm.areaUnit.toLowerCase()} total
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.center}>
              ...
              <Text style={styles.emptyText}>No crops yet. Add your first crop below.</Text>
            </View>
          }
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              <View style={styles.cardBody}>
                ...
                <Text style={styles.cardSubtitle}>
                  {item.area} {item.areaUnit.toLowerCase()}
                </Text>
              </View>
          ...
          <ScrollView style={styles.form} keyboardShouldPersistTaps="handled">
            <View>
              ...
              <View style={styles.formActions}>
                ...
                <TouchableOpacity style={styles.primaryButtonWrap} onPress={onAddCrop} disabled={isSaving} activeOpacity={0.85}>
                  <LinearGradient colors={theme.gradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.primaryButton}>
                    {isSaving ? <ActivityIndicator color="#fff" /> : <Text style={styles.primaryButtonText}>Save Crop</Text>}
                  </LinearGradient>
                </TouchableOpacity>
              </View>
            </View>
