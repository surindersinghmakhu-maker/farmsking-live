...
import { MarketRatesService, toEnglishCropName } from '../backend/src/modules/market-rates/market-rates.service';
...
async function runTest() {
  ...
  // Create a test farmer user if needed
  ...
  const bill1 = await prisma.saleBill.create({
    data: {
      farmerId: farmer.id,
  ...
  // Also record in MarketRate table as done by sale-bills.service.ts
  ...
  const bill2 = await prisma.saleBill.create({
    data: {
      farmerId: farmer.id,
  ...
  if (
    ...
    console.log('\n❌ VERIFICATION FAILED! Check rate pool values.');
  }
